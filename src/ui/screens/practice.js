export function renderPractice(state){
  ...
  return `<section class="stack-lg guided-practice">
    ...
    <div class="practice-intent-grid">
      ...
      ${intentCard({
        route:"review",icon:"↺",title:"Повторить знакомое",
        text:"Активно вспомнить выражения, которые подошли к повторению, по их слабейшему навыку.",
        meta:due?`${due} к повторению`:"Срочных повторений нет",quiet:due===0
      })}
      ${intentCard({
        route:"real-life",icon:"◎",title:"Мне нужно это сейчас",
        text:"Опишите реальную ситуацию и получите естественную фразу, которую можно сразу потренировать и сохранить.",
        meta:"Real Life",accent:true
      })}
      ${intentCard({
        route:"teacher",icon:"Aa",title:"Разобрать грамматику",
        text:grammarMistakes
          ?"Сфокусироваться на реальной повторяющейся ошибке из вашей собственной практики."
          :"Короткое объяснение одной конструкции → активное применение, без учебника грамматики.",
        meta:grammarMistakes?`${grammarMistakes} активных ошибок`:"Grammar Focus"
      })}
    </div>

    <section class="practice-tools" aria-labelledby="practice-tools-title">
      <div class="section-heading guided-practice-heading">
        <div><p class="eyebrow">ДОПОЛНИТЕЛЬНЫЕ ИНСТРУМЕНТЫ</p><h3 id="practice-tools-title">Когда хочется выбрать формат вручную</h3></div>
      </div>
      <div class="practice-tool-list">
        <button class="practice-tool-row" type="button" data-route="teacher">
          <span><strong>Сформировать своё упражнение</strong><small>AI Teacher · конкретная тема, ситуация или конструкция</small></span>
          <span aria-hidden="true">→</span>
        </button>
        ${!hasSession?`
          <div class="practice-tool-row is-disabled" aria-disabled="true">
            <span><strong>Сегодняшнее занятие</strong><small>Session Engine ещё подготавливает локальную сессию</small></span>
            <span aria-hidden="true">…</span>
          </div>
        `:""}
      </div>
